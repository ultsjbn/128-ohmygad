...
import {
  User, Mail, Phone, BookOpen, MapPin, Camera,
  Save, CheckCircle2, AlertCircle, ChevronLeft,
  GraduationCap, Heart, Building2, Users,
} from "lucide-react";
...
export default function StudentProfilePage() {
  ...
  const [tab, setTab] = useState<"personal" | "academic" | "identity">("personal");
  ...
  async function handleSave() {
    ...
    try {
      ...
      const { error } = await supabase
        .from("profile")
        .upsert(
          { ...rest, student_num: rest.student_num === "" ? null : Number(rest.student_num) },
          { onConflict: "id" }
        );
    ...
    <div className="min-h-screen bg-fractal-bg-body-default">
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 flex flex-col gap-5">
        ...
        <div className="bg-fractal-bg-body-white border-1 border-fractal-border-default rounded-m p-6 shadow-brutal-1">
          ...
          {tab === "academic" && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Field label="Student Number" icon={<BookOpen size={15} />}>
                <input
                  className={inputCls()}
              ...
              <Field label="Year Level" icon={<GraduationCap size={15} />}>
                <select className={inputCls()} value={profile.year_level} onChange={set("year_level")}>
                  <option value="">Select year level</option>
                  {YEAR_LEVELS.map((y) => <option key={y} value={y}>{y}</option>)}
                </select>
              </Field>

              <Field label="Program" icon={<BookOpen size={15} />}>
                <input
                  className={inputCls()}
